 ...
 * @description: 线路终点站常态化站前折返道岔故障-不具备站后折返Handler
 * @author: liyunlong
 * @date: 2021/12/28 17:40
 */
@Slf4j
@Service
public class FrontFailureNotHasBehindTurnHandler implements AlarmInfoMessageHandler {
    ...
    public void handle(AlarmInfo alarmInfo) {
        ...
        // 2:故障恢复
        int alarmState = alarmInfo.getAlarmState();
        if (NumConstant.TWO.equals(alarmState)) {
            ...
            strategy.pushRecoveryAdm(info);
            return;
        }
        log.info("道岔故障-线路终点站常态化站前折返道岔故障,不具备站后折返录入参数:{}", alarmInfo);
        // 设置推荐指令步骤
        ...
        ConStationDto conStationDto =
                conStationInfoService.getStationByConStationId(Integer.parseInt(alarmInfo.getAlarmConStation()));
