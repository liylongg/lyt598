...
/**
 * @Description 车门故障-全列车门无法打开
 * @Author yhf
 * @Date 2022/6/15 14:15
 **/
@Slf4j
@Component
public class AllTrainDoorCannotOpenHandler implements AlarmInfoMessageHandler {
    ...
    public void handle(AlarmInfo alarmInfo) {
        ...
        if (alarmInfo.getAlarmState() == 1){
            ...
            if (NumStrConstant.ONE.equals(trainDoorNotOpenVobc)) {
                BasicCommonCacheUtils.hPut(Cache.TRAIN_DOOR_VOBC_STATE, alarmInfo.getTrainId(), "5-1");
                log.info("整侧车门无法打开故障");
            }else{
                // 车门屏蔽门故障报警
