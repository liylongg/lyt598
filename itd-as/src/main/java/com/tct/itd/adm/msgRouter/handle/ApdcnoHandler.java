...
/**
 * @classname: ApdcnoHandler
 * @description: 整侧站台门无法打开(Apdcno : All platform door can not open)
 * @author: liyunlong
 * @date: 2021/9/30 11:02
 */
@Service
@Slf4j
public class ApdcnoHandler implements AlarmInfoMessageHandler {
    ...
    public void handle(AlarmInfo alarmInfo) {
        ...
        log.info("站台门故障-整侧站台门无法打开,故障信息:{}", alarmInfo);
        ...
        // 获取第一次推荐指令执行单元信息
        List<AidDesSubStepEntity> aidDesSubStepList = aidDesSubStepUtils.getAidDesSubStep(alarmInfo.getAlarmTypeDetail(), ONE_NUM, ZERO_NUM);
        //替换站名
