...
/**
 * @classname: BehindFailureChangeAdmHandler
 * @description: 终端站后折返道岔故障可换轨Handler
 * @author: liyunlong
 * @date: 2023/3/21 10:13
 */
@Slf4j
@Service
public class BehindFailureChangeAdmHandler implements AuxiliaryDecisionHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        switch (executeStep) {
            case 1:
                log.info("终端站后道岔故障可换轨：执行第一次推荐指令:{}", auxiliaryDecision);
                doAidDecisionOne(auxiliaryDecision);
                break;
            case 2:
                log.info("终端站后道岔故障可换轨：执行第二次推荐指令:{}", auxiliaryDecision);
                doAidDecisionTwo(auxiliaryDecision);
                break;
            case 3:
                log.info("终端站后道岔故障可换轨：执行第三次推荐指令:{}", auxiliaryDecision);
                doAidDecisionThree(auxiliaryDecision);
                break;
            case 4:
                log.info("终端站后道岔故障可换轨：执行第四次推荐指令:{}", auxiliaryDecision);
    ...
    public String channel() {
        return AlarmTypeConstant.BEHIND_FAILURE_CHANGE;
    }
