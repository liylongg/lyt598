...
/**
 * @Description 单车门无法关闭-运行图预览
 * @Author zhangyinglong
 * @Date 2021/6/2 14:27
 */
@Slf4j
@Service
public class OnlyDoorCannotCloseStrategy extends AbstractPrePlanRunGraphStrategy implements IPrePlanRunGraphStrategy {
    ...
    public String getEndAlarmTime(AlarmInfo alarmInfo) {
        if(IidsConstPool.EXECUTE_STEP_3 == alarmInfo.getExecuteStep() || IidsConstPool.EXECUTE_STEP_4 == alarmInfo.getExecuteStep()){
            //推送10分钟
    ...
    public List<AlgStrategyResult> listAdjustStrategy(AlarmInfo alarmInfo) {
        ...
        try {
            if(IidsConstPool.EXECUTE_STEP_0_2 == alarmInfo.getExecuteStep()){
            ...
            }else if(IidsConstPool.EXECUTE_STEP_3 == alarmInfo.getExecuteStep() || IidsConstPool.EXECUTE_STEP_4 == alarmInfo.getExecuteStep()){
                resultList = aidDecisionExecService.adjustRunGraphDoorAlg(alarmInfo);
            }else if(IidsConstPool.EXECUTE_STEP_5 == alarmInfo.getExecuteStep()){
                //复制前端选择的终点站掉线
                alarmInfo.setAlarmState(AlarmStateEnum.TRAIN_DOOR_END_DROP_LINE_HOLD.getCode());
                //终点站回段
                List<AlgStrategyResult> result1 = aidDecisionExecService.adjustRunGraphDoorAlg(alarmInfo);
                result1.forEach(algStrategyResult -> {
                    algStrategyResult.setCaseCode(AlarmStateEnum.TRAIN_DOOR_END_DROP_LINE_HOLD.getCode());
                    algStrategyResult.setCaseDesc("列车终点站回段");
                });
                //晚点调整继续运行传0，传alarminfo空对象
                alarmInfo.setAlarmState(0);
                List<AlgStrategyResult> result2 = aidDecisionExecService.adjustRunGraphDoorAlg(alarmInfo);
                result2.forEach(algStrategyResult -> {
                    algStrategyResult.setCaseCode(AlarmStateEnum.LATE_ADJUSTMENT.getCode());
                    algStrategyResult.setCaseDesc("列车继续运行");
                });
                resultList.addAll(result1);
                resultList.addAll(result2);
            }else if(IidsConstPool.EXECUTE_STEP_6 == alarmInfo.getExecuteStep()){
    ...
    public PrePlanRunGraph previewRunGraph(AlarmInfo alarmInfo) {
        ...
        try {
            if(IidsConstPool.EXECUTE_STEP_0_2 == alarmInfo.getExecuteStep()){
                ...
                //晚点传空对象
                if(alarmInfo.getCaseCode() == AlarmStateEnum.LATE_ADJUSTMENT.getCode()){
                    alarmInfo.setAlarmState(0);
                }else{
                    alarmInfo.setAlarmState(alarmInfo.getCaseCode());
                }
    ...
    public String strategy() {
        return AlarmTypeConstant.ONLY_DOOR_CANNOT_CLOSE;
    }
}
