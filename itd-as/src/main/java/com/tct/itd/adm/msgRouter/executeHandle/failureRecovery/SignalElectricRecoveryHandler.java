...
/**
 * @Description :信号电源-故障恢复handler
 * @Author : zhoukun
 * @Date : Created in 2022-04-19
 */
@Slf4j
@Component
public class SignalElectricRecoveryHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        BasicCommonCacheUtils.delKey(Cache.SINGAL_ELECTRIC_PREVIEW_TIME_RECORD);
        //获取故障信息
        ...
        if (auxiliaryDecision.getExecuteStep() != IidsConstPool.EXECUTE_STEP_0_99) {
            ...
            // 如果是第一次故障恢复则结束生命流程
            if (auxiliaryDecision.getExecuteStep() == IidsConstPool.EXECUTE_STEP_0_1 && auxiliaryDecision.getAidDesSubStepDtoList().size()!=4) {
            ...
            else if (auxiliaryDecision.getExecuteStep() == IidsConstPool.EXECUTE_STEP_0_1 && auxiliaryDecision.getAidDesSubStepDtoList().size()==4) {
                // 禁用故障续报按钮
                admAlertInfoService.updateReportById(alarmInfo.getTableInfoId(), 2);
            ...
            //推荐指令入库
            log.info("信号电源,放弃故障恢复推荐指令:{}", alarmInfo);
            alarmInfo.setEndLife(false);
            // 禁用故障续报按钮
            admAlertInfoService.updateReportById(alarmInfo.getTableInfoId(), 2);
            //将故障恢复的状态为1的置为2，按钮置灰
