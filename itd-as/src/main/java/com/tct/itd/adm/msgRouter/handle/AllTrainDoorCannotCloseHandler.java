...
/**
 * @Description 车门故障-全列车门无法关闭
 * @Author zyl
 * @Date 2021/5/17 22:20
 **/
@Slf4j
@Component
public class AllTrainDoorCannotCloseHandler implements AlarmInfoMessageHandler {
    @Resource
    ...
    public void handle(AlarmInfo alarmInfo) {
        ...
        if (alarmInfo.getAlarmState() == 1) {
            ...
            log.info("车门故障-全列车门无法打开,产生故障:{}", alarmInfo);
            // 设置站台,停车区域编号信息
            ...
            //更新流程图
            alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(10,41));
        }
    }
