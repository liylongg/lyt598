...
import com.tct.itd.adm.service.AxleCounterPreUpdateFlowchartsService;
...
@Slf4j
@Component
public class AxleCounterSwitchAdmHandler implements AuxiliaryDecisionHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        switch (executeStep) {
            ...
            case IidsConstPool.EXECUTE_STEP_3:
                log.info("执行第三次推荐指令:{}", auxiliaryDecision);
                doThirdAuxiliaryDecision(auxiliaryDecision);
                break;
            case IidsConstPool.EXECUTE_STEP_4:
                log.info("执行第四次推荐指令:{}", auxiliaryDecision);
                doFourthAuxiliaryDecision(auxiliaryDecision);
                break;
            case IidsConstPool.EXECUTE_STEP_5:
                log.info("执行第五次推荐指令:{}", auxiliaryDecision);
                doFiveAuxiliaryDecision(auxiliaryDecision);
                break;
            default:
        ...
        //更新流程图
        axleCounterUpdateFlowchartsService.updateSwitchExecute(auxiliaryDecision.getTableInfoId(), auxiliaryDecision.getExecuteStep());
    }
    ...
    private void doAidDecisionRecovery(AuxiliaryDecision auxiliaryDecision) {
        ...
        if (auxiliaryDecision.getExecuteStep() != IidsConstPool.EXECUTE_STEP_0_99) {
            ...
            //更新流程图
            axleCounterUpdateFlowchartsService.updateSwitchExecute(alarmInfo.getTableInfoId(), auxiliaryDecision.getExecuteStep());
        } else { //执行放弃故障恢复推荐指令
            //推荐指令入库
    ...
    /***
     * @description 执行第三次推荐指令
     * @date 2022/1/15 14:58
     * @author kangyi
     * @param auxiliaryDecision
     * @return: void
     */
    private void doThirdAuxiliaryDecision(AuxiliaryDecision auxiliaryDecision) {
        //获取告警信息
        AlarmInfo alarmInfo = admAlertInfoSubService.queryByInfoId(auxiliaryDecision.getTableInfoId());
        Assert.notNull(alarmInfo, "生命周期已结束，推荐指令流程结束");
        log.info("执行第三次推荐指令,alarmInfo:{}", JsonUtils.toJSONString(alarmInfo));
        //更新方案状态为已执行
        alarmInfoOprtService.updateStatus(alarmInfo.getTableInfoId(), alarmInfo.getTableBoxId2(), AlarmInfoEnum.EXECUTE_END_1.getCode());
        //执行推荐指令，调用对应handle处理
        aidDecSubStepRouter.aidDecSubStepRouter(alarmInfo, auxiliaryDecision);
        ...
        alarmInfo.setExecuteStep(auxiliaryDecision.getExecuteStep());
        admAlertInfoSubService.updateById(alarmInfo);
        Integer stationConfirm = (Integer) BasicCommonCacheUtils.hGet(Cache.FLOWCHART_FLAG, Cache.STATION_CONFIRM_FLAG);
        //车站选择道岔强转至行车方向，添加缓行标志
        if (Objects.equals(stationConfirm, StationConfirmEnum.FIRST_CONFIRM.getCode())) {
            BasicCommonCacheUtils.set(Cache.AXLE_COUNTER_SLOWLY_GRAPH_SIGN, "0");
        }
    }

    /***
     * @description 执行第四次推荐指令
     * @date 2022/1/15 14:58
     * @author kangyi
     * @param auxiliaryDecision
     * @return: void
     */
    private void doFourthAuxiliaryDecision(AuxiliaryDecision auxiliaryDecision) {
        //获取告警信息
        AlarmInfo alarmInfo = admAlertInfoSubService.queryByInfoId(auxiliaryDecision.getTableInfoId());
        Assert.notNull(alarmInfo, "生命周期已结束，推荐指令流程结束");
        log.info("执行第四次推荐指令,alarmInfo:{}", JsonUtils.toJSONString(alarmInfo));
        //更新方案状态为已执行
        alarmInfoOprtService.updateStatus(alarmInfo.getTableInfoId(), alarmInfo.getTableBoxId2(), AlarmInfoEnum.EXECUTE_END_1.getCode());
        //执行推荐指令，调用对应handle处理
        aidDecSubStepRouter.aidDecSubStepRouter(alarmInfo, auxiliaryDecision);
    ...
    /***
     * @description 执行第五次推荐指令
     * @date 2022/1/15 14:58
     * @author kangyi
     * @param auxiliaryDecision
     * @return: void
     */
    private void doFiveAuxiliaryDecision(AuxiliaryDecision auxiliaryDecision) {
        //获取告警信息
        AlarmInfo alarmInfo = admAlertInfoSubService.queryByInfoId(auxiliaryDecision.getTableInfoId());
        Assert.notNull(alarmInfo, "生命周期已结束，推荐指令流程结束");
        log.info("执行第五次推荐指令,alarmInfo:{}", JsonUtils.toJSONString(alarmInfo));
        //更新方案状态为已执行
        alarmInfoOprtService.updateStatus(alarmInfo.getTableInfoId(), alarmInfo.getTableBoxId2(), AlarmInfoEnum.EXECUTE_END_1.getCode());
        //执行推荐指令，调用对应handle处理
        aidDecSubStepRouter.aidDecSubStepRouter(alarmInfo, auxiliaryDecision);
        ...
        alarmInfo.setExecuteStep(auxiliaryDecision.getExecuteStep());
        admAlertInfoSubService.updateById(alarmInfo);
        String preFlag = (String) BasicCommonCacheUtils.hGet(Cache.FLOWCHART_FLAG, Cache.PRE_SUCCESS_FLAG);
        //没有预复位成功，预复位失败第五次推荐指令添加缓行标志
    ...
    public String channel() {
        return AlarmTypeConstant.AXLE_COUNTER_SWITCH;
    }
}
