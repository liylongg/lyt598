...
import com.tct.itd.adm.msgRouter.executeHandle.failureRecovery.OnlyDoorCannotCloseAdmFailureRecoveryHandler;
...
@Slf4j
@Component
public class OnlyDoorCannotCloseAdmHandler implements AuxiliaryDecisionHandler {
    ...
    @Resource
    private OnlyDoorCannotCloseAdmFailureRecoveryHandler onlyDoorCannotCloseAdmFailureRecoveryHandler;
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        if(executeStep == 1){
            ...
            alarmFlowchartService.setExecuteFlags(auxiliaryDecision.getTableInfoId(), Lists.newArrayList(3,4,5,6,7));
        } else if (executeStep == 2) {
            doGraphADM(auxiliaryDecision);
            alarmFlowchartService.setExecuteFlags(auxiliaryDecision.getTableInfoId(), Lists.newArrayList(27,28));
        }else if (executeStep == 3){
            doThreeADM(auxiliaryDecision);
            alarmFlowchartService.setExecuteFlags(auxiliaryDecision.getTableInfoId(), Lists.newArrayList(12,13,14));
        } else if (executeStep == 4) {
            doFourADM(auxiliaryDecision);
            alarmFlowchartService.setExecuteFlags(auxiliaryDecision.getTableInfoId(), Lists.newArrayList(16,17,18));
        } else if (executeStep == 5) {
            doFiveADM(auxiliaryDecision);
            alarmFlowchartService.setExecuteFlags(auxiliaryDecision.getTableInfoId(), Lists.newArrayList(21,22,23,24));
        } else if (executeStep < 0) {
            onlyDoorCannotCloseAdmFailureRecoveryHandler.handle(auxiliaryDecision);
        }else {
