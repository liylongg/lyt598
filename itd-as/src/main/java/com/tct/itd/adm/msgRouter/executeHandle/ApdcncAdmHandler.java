...
/**
 * @Classname: ApdcncAdmHandler
 * @Description: 整侧站台门无法关闭-推荐指令(Apdcnc:All platform door cannot close)
 * @Author: liyunlong
 * @Date: 2021/10/20 14:09
 */
@Service
@Slf4j
public class ApdcncAdmHandler implements AuxiliaryDecisionHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        //更新流程图
        platformDoorUpdateFlowchartsService.APDCCExecute(auxiliaryDecision.getTableInfoId(), auxiliaryDecision.getExecuteStep());
    }
    ...
    private void doAidDecisionRecovery(AuxiliaryDecision auxiliaryDecision) {
        ...
        if (auxiliaryDecision.getExecuteStep() != IidsConstPool.EXECUTE_STEP_0_99) {
            ...
            log.info("整侧站台门无法关闭,故障解除:{}", JsonUtils.toJSONString(alarmInfo));
            ...
            //更新流程图
            platformDoorUpdateFlowchartsService.APDCCExecute(alarmInfo.getTableInfoId(), auxiliaryDecision.getExecuteStep());
        } else { //执行放弃故障恢复推荐指令
            //推荐指令入库
    ...
    private void doAidDecisionOne(AuxiliaryDecision auxiliaryDecision) {
        ...
        Assert.notNull(alarmInfo, "生命周期已结束，推荐指令流程结束");
        log.info("站台门故障-整侧车门无法关闭,执行第一次推荐指令,alarmInfo:{}", alarmInfo);
        // 更新推荐指令方案状态已执行
