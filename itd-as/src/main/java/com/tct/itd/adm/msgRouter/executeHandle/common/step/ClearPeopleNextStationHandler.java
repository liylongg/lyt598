...
/**
 * @Description 执行推荐指令-下一站清人
 * @Author zhoukun
 * @Date 2022/05/12
 */
@Service("clearPeopleNextStation")
@Slf4j
public class ClearPeopleNextStationHandler implements AidDecSubStepHandler {
    @Resource
        ...
        if (BasicCommonCacheUtils.exist(Cache.TRAIN_NUMBER_ADJUST)) {
            for (Map.Entry<Object, Object> m : BasicCommonCacheUtils.hmget(Cache.TRAIN_NUMBER_ADJUST,TrainNumberAdjust.class,List.class).entrySet()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    //发送清客指令
                    String platFormId = alarmUtil.getPlatformIdByTrainTraceInSection(alarmInfo.getTrainId()) + "";
