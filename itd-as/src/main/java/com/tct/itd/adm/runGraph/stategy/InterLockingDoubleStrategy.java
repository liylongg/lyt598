...
@Slf4j
@Service
public class InterLockingDoubleStrategy extends AbstractPrePlanRunGraphStrategy implements IPrePlanRunGraphStrategy {
    @Resource
    ...
    public String getEndAlarmTime(AlarmInfo alarmInfo) {
        //调60分钟图
        int tractionPowerTime = Integer.parseInt(SysParamKit.getByCode(IidsSysParamPool.INTER_LOCK_PREVIEW_TIME));
    ...
    public List<AlgStrategyResult> listAdjustStrategy(AlarmInfo alarmInfo) {
        ...
        //记录推图时间
        BasicCommonCacheUtils.hPut(Cache.INNER_LOCK_PREVIEW_TIME_RECORD,alarmInfo.getAlarmConStation(),System.currentTimeMillis());
        ...
        try {
           algStrategyResultList = aidDecisionExecService.adjustRunGraphLockAlg(alarmInfo);
            ...
            log.error("联锁双机取调整方案列表出错,异常信息:【{}】", e.getMessage());
