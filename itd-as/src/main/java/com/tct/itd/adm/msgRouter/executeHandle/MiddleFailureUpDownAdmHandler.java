...
/**
 * @classname: MiddleFailureUpDownAdmHandler
 * @description: 线路中间站道岔故障(非小交路折返站 - 影响上下行)
 * @author: liyunlong
 * @date: 2023/3/2 16:51
 */
@Slf4j
@Service
public class MiddleFailureUpDownAdmHandler implements AuxiliaryDecisionHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        switch (executeStep) {
            case 1:
                log.info("线路终点站常态化站后折返道岔故障,不具备站前折返：执行第一次推荐指令:{}", auxiliaryDecision);
                doAidDecisionOne(auxiliaryDecision);
                break;
            case 2:
                log.info("线路终点站常态化站后折返道岔故障,不具备站前折返：执行第二次推荐指令:{}", auxiliaryDecision);
                doAidDecisionTwo(auxiliaryDecision);
                break;
            case 3:
                log.info("线路终点站常态化站后折返道岔故障,不具备站前折返：执行第三次推荐指令:{}", auxiliaryDecision);
                doAidDecisionThree(auxiliaryDecision);
                break;
            case 4:
                log.info("线路终点站常态化站后折返道岔故障,不具备站前折返：执行第四次推荐指令:{}", auxiliaryDecision);
    ...
    public String channel() {
        return AlarmTypeConstant.MIDDLE_FAILURE_UP_DOWN;
    }
