...
import com.tct.itd.adm.msgRouter.executeHandle.failureRecovery.OnlyDoorCannotOpenAdmFailureRecoveryHandler;
...
@Slf4j
@Component
public class OnlyDoorCannotOpenAdmHandler implements AuxiliaryDecisionHandler {
    ...
    @Resource
    private OnlyDoorCannotOpenAdmFailureRecoveryHandler onlyDoorCannotOpenAdmFailureRecoveryHandler;
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        if(executeStep == 1){
            ...
            onlyDoorCannotOpenAdmFailureRecoveryHandler.handle(auxiliaryDecision);
        }else {
    ...
    private void doGraphADM(AuxiliaryDecision auxiliaryDecision) {
        ...
        if(alarmFlowchartService.nodeAlreadyRefreshed(alarmInfo.getTableInfoId(), 7)){
            //更新流程图
            alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(11,12));
        }else{
            //更新流程图
    ...
    private void doOffHoldADM(AuxiliaryDecision auxiliaryDecision) {
        ...
        //更新流程图
        alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(3,4,5,6));
    }
