...
@Slf4j
@Component
public class AxleCounterNotSwitchAdmHandler implements AuxiliaryDecisionHandler {
    ...
    @Resource
    private AxleCounterPreUpdateFlowchartsService axleCounterPreUpdateFlowchartsService;
    @Resource
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        //更新流程图
        axleCounterPreUpdateFlowchartsService.updateNotSwitch(auxiliaryDecision.getTableInfoId(), auxiliaryDecision.getExecuteStep());
    }
    ...
    private void doAidDecisionRecovery(AuxiliaryDecision auxiliaryDecision) {
        ...
        if (auxiliaryDecision.getExecuteStep() != IidsConstPool.EXECUTE_STEP_0_99) {
            ...
            //更新流程图
            axleCounterPreUpdateFlowchartsService.updateNotSwitch(alarmInfo.getTableInfoId(), auxiliaryDecision.getExecuteStep());
        } else { //执行放弃故障恢复推荐指令
            //推荐指令入库
