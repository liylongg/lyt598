...
/**
 * @Description 全列车门无法关闭-缓行-执行故障恢复推荐指令
 * @Author yuelei
 * @Date 2022/8/4 19:17
 **/
...
@Slf4j
@Component
public class AllTrainDoorCannotCloseSlowRecoveryHandler {
    ...
    public AllTrainDoorCannotCloseSlowRecoveryHandler() {
    }
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision)  throws RuntimeException {
        ...
        if (executeStep != IidsConstPool.EXECUTE_STEP_0_99) {
            ...
            if(auxiliaryDecision.getExecuteStep() == IidsConstPool.EXECUTE_STEP_0_1){
                ...
                //更新流程图
                alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(27,28,29,30));
            }else{
                admAlertInfoSubService.updateExecuteEndAndStep(alarmInfo);
                //更新流程图
                alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(31,32,33,34));
            }
