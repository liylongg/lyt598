...
/**
 * @classname: SFMidSingleAdmHandler
 * @description: 道岔故障-中间道岔故障-影响单行AdmHandler
 * @author: liyunlong
 * @date: 2021/12/29 11:26
 */
@Slf4j
@Component
public class SFMidSingleAdmHandler implements AuxiliaryDecisionHandler {
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        switch (executeStep) {
            case 1:
                log.info("道岔故障-中间道岔故障-影响单行：执行第一次推荐指令:{}", auxiliaryDecision);
                doAidDecisionOne(auxiliaryDecision);
                break;
            case 2:
                log.info("道岔故障-中间道岔故障-影响单行：执行第二次推荐指令:{}", auxiliaryDecision);
                doAidDecisionTwo(auxiliaryDecision);
                break;
            case 3:
                log.info("道岔故障-中间道岔故障-影响单行：执行第三次推荐指令:{}", auxiliaryDecision);
                doAidDecisionThree(auxiliaryDecision);
                break;
            case 4:
                log.info("道岔故障-中间道岔故障-影响单行：执行第四次推荐指令:{}", auxiliaryDecision);
    ...
    private void doAidDecisionTwo(AuxiliaryDecision auxiliaryDecision) {
        ...
        alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(12, 13, 14, 15, 16, 17, 18, 19));
    }
    ...
    private void doAidDecisionThree(AuxiliaryDecision auxiliaryDecision) {
        ...
        alarmInfoOprtService.insertAdmAlertDetail(UidGeneratorUtils.getUID(), alarmInfo.getTableInfoId(), "系统产生运行图方案"
                , "2", "运行图方案选择", 1);
        // 存储至redis,等待定时任务根据客户端选择的策略进行调图
        BasicCommonCacheUtils.hPut(Cache.SWITCH_TWENTY_PREVIEW_SIGN,alarmInfo.getSwitchNo(),System.currentTimeMillis());
        alarmFlowchartService.setExecuteFlags(alarmInfo.getTableInfoId(), Lists.newArrayList(22, 23, 24, 25));
    }
