...
import com.tct.itd.adm.msgRouter.executeHandle.failureRecovery.SignalElectricRecoveryHandler;
...
/**
 * @Description : 信号电源故障执行推荐指令
 * @Author : zhoukun
 * @Date : Created in 2022/4/19
 */
@Slf4j
@Component
public class SignalElectricSourceAdmHandler implements AuxiliaryDecisionHandler {
    ...
    @Resource
    private SignalElectricRecoveryHandler recoveryHandler;
    ...
    public void handle(AuxiliaryDecision auxiliaryDecision) throws RuntimeException {
        ...
        if (executeStep==1){
            log.info("信号电源故障：执行第一次推荐指令:{}", auxiliaryDecision);
            doAidDecisionOne(auxiliaryDecision);
        }else if(executeStep==2){
            log.info("信号电源故障：执行第二次推荐指令:{}", auxiliaryDecision);
            doAidDecisionTwo(auxiliaryDecision);
        }else if(executeStep<0){
            log.info("信号电源故障：执行故障恢复推荐指令:{}", auxiliaryDecision);
    ...
    private void doAidDecisionOne(AuxiliaryDecision auxiliaryDecision) {
        ...
        appPushService.sendWebNoticeMessageToAny(
                new WebNoticeDto(WebNoticeCodeConst.REFRESH_NULL_MSG, "0", "刷新信号电源故障按钮"));
